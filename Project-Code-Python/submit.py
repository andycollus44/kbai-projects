...
def main():
    ...
    parser.add_argument('--assignment', choices=['P1', 'P2', 'P3', 'error-check', 'error-check-2', 'error-check-3'],
                        required=True)
    ...
    for file in files:
        if ext(file) in forbidden_exts:
            ...
            print("Warning: you're submitting a", ext(file),
        ...
        if 'Execution' in result.get('Error', {}):
            ...
            if ('Problems' in result) & ('Sets' in result):
                ...
                for key in problems:
                    ...
                    print(','.join(['"' + problem['Problem'] + '"', success, problem['Correct Answer'],
                                    problem['Agent\'s Answer']]))
